...
for frog in Oophaga Rhinella Ranitomeya 
do

	# make protein sequences from nucloetide fasta
	python2.7 translate_fasta.py ../input_sequences/LWS_all.fasta > output/LWS_all_${frog}_tblastn.translated.fasta

	# run a blast on the SWS2 sequences using each databse
	python2.7 run_blast.LWS.protein.py ${frog} 7 

	# make a fasta that now includes the results of the blast search, using hits with evals < 1e-10
	python2.7 make_fasta_from_tblastn7.py 1e-10 output/LWS_results.${frog}.7.tblastn.txt ../genome_sequences/${frog}.fa >> output/LWS_all_${frog}_tblastn.translated.fasta 

	# align results of BLAST to SWS2 sequences
	mafft output/LWS_all_${frog}_tblastn.translated.fasta  >  output/LWS_all_${frog}_tblastn.translated.mafft.fasta 
	...
	python2.7 get_putative_ortholog_names.py output/Gene_annot_results.${frog}.LWS.6.tblastn.txt ../genome_sequences/Nanorana.fa > output/most_similar_gene_in_nanorana_LWS_${frog}.txt
done